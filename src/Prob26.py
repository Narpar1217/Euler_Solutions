...
def GetCycleLength(n):
    """
    For a positve integer n, returns the length of the
    repeating cycle of 1/n.
    """
...
def IsFullReptend(p):
    """Returns true if prime p is a full reptend prime."""
...
#-----------------------------------------------------------------------------
#Explanation:
# A full reptend prime is a prime for which (10**(p - 1) - 1) / p
# produces a cyclical number of length p - 1 that is equivalent to
# the recurring cycle of 1 / p. A prime number p that is not full repted
# has a cycle length that is a factor of p - 1.
#
# The period of a composite number c is <= c - 2.
#
# The answer is found by stepping backwards from 999 until a full
# reptend prime is found, and returning the maximum cycle length
# found up to that point.
#
# The correct result is guaranteed, as a non-full reptend
# prime can not have a period > itself / 2, no number below a
# full reptend prime can have a longer period, and the cycle
# length of any greater composite numbers are found explicitly.
def Prob26():
