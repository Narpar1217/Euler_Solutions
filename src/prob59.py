...
def decrypt(key, data):
    """
    Generator; yield next character of data XOR'd with key.
...
def get_most_likely_key(testData):
    """
    Return key most likely to accurately decrypt testData, or None if no
    such key is found.
    ...
    for k1 in xrange(start, stop):
        for k2 in xrange(start, stop):
            for k3 in xrange(start, stop):
                testKey = [k1, k2, k3]
                matches = test_key(testKey, testData, commonWords,
                                   highestMatch)
...
def prob59(filename):
    ...
    # Get reasonably-sized slice from middle of encrypted text to test keys
    # against (50 characters, or half the length of encrypted if
    # len(encrypted) < 100)
    ...
    # Find key
    key = get_most_likely_key(testSlice)
...
def test_key(key, testData, compareTo, minMatch, maxWordLen=10):
    """
    Return number of words in testData that are in compareTo, after testData
