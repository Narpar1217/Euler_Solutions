...
PROBLEM DESCRIPTION:
  The 11K text file, Prob89_roman.txt, contains one thousand numbers written
  in valid, but not necessarily minimal, Roman numerals; That is, they are
  arranged in descending units and obey the subtractive pair rule.
...
###############################################################################
def int_to_roman(n):
    """Return integer n as minimum-length roman numeral string"""
    rom = ''
    ...
    for deg, digit in enumerate(get_digits(n)):
...
def prob89(filename):
    ...
    with open(filename, 'r') as f:
        for line in f:
            romLong = line.strip()
            romMin = int_to_roman(roman_to_int(romLong))
...
def roman_to_int(rom):
    """
    Return roman numeral rom as integer. Assumes rom is a valid roman numeral,
    though it need not be minimal-length (i.e. the following are all valid
    input values for the number 12: XII, VVII, VIIIIIII, IIIIIIIIIIII).
